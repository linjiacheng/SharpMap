...
namespace GeoAPI.CoordinateSystems.Transformations
{
    ...
    public class GeometryTransform
    {
        ...
        public static Envelope TransformBox(Envelope box, IMathTransform transform)
        {
            ...
            var corners = new[] {
                transform.Transform(new [] { box.MinX, box.MinY }),
                transform.Transform(new [] { box.MinX, box.MaxY }),
                transform.Transform(new [] { box.MaxX, box.MinY }),
                transform.Transform(new [] { box.MaxX, box.MaxY })};

            var result = new Envelope(new Coordinate(corners[0][0], corners[0][1]));
            for (var i = 1; i < 4; i++)
                result.ExpandToInclude(new Coordinate(corners[i][0], corners[i][1]));
            return result;
        }
